...
Thus, the increase in CGE amongst hub interneurons cannot be simply explained by their anatomical similarity among interneurons.
...
To ensure that our measure of CGE is not biased by differences in the relative proportions of expression annotations, we conducted a sensitivity analysis in which we compared the $r_\phi$ metric, Eq.~\eqref{eq:rphi}, with alternative methods for quantifying correlations between binary vectors: the
...
Because all vectors are independent random binary strings, the mean CGE value should not show any systematic variation, which would indicate bias.
The mean square contingency coefficient, $r_\phi$ (Fig.~\ref{fig:S_propOnes}A) and our own novel CGE matching index (Fig.~\ref{fig:S_propOnes}D) show no systematic bias to the proportion of ones in the vector (varying within $\approx 10^{-3}$ and $\approx 0.5$ respectively), whereas Yule's $Q$ shows a negative bias for small annotation proportions (Fig.~\ref{fig:S_propOnes}B), while the Jaccard index shows a strong positive bias across the full range (Fig.~\ref{fig:S_propOnes}C).
Based on these numerical experiments, we selected Pearson's mean square contingency coefficient, $r_\phi$, here, to ensure that changes in CGE were due to matching structure and not simply the number of gene annotations.
...
\begin{table}[]
\centering
\caption{Biological processes GO categories showing increased CGE for connected neurons compared to unconnected neurons.
...
\centering
\caption{
Biological processes GO categories showing increased CGE for links involving hubs (rich, feed-in and feed-out) compared to peripheral links.
...
\textbf{Dependence of binary CGE metrics on the proportion of positive annotations.}
...
\caption{
\textbf{Correlated gene expression, as measured by a positive matching probability index, Eq.~(\ref{eqn:positiveMatch}).}
