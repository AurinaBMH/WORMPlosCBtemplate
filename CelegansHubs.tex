...
\section*{Abstract}
Highly connected hubs of connectomes have been shown to be densely inter-connected, forming a costly but central backbone for neuronal communication.
Adding to this picture, previous work has characterized the transcriptional similarity of hub regions in the mesoscale mouse connectome, suggesting that hubs may also be distinctive in their gene expression patterns.
Given the importance of hubs across scales and species, here we test whether similar signatures of hub connectivity are also present in the fully mapped neuronal nervous system of \emph{C. elegans}.
We assembled open data, on the chemical and electrical connectivity structure of \emph{C. elegans}, matched to a binary gene expression signature across 948 genes for each neuron, and computed a correlated gene expression score for each pair of neurons according to their transcriptional similarity.
We characterize the decreasing spatial dependence of neuronal connectivity and correlated gene expression, showing that both are consistent with prior patterns in other species.
We then confirmed the rich-club connectivity of \emph{C. elegans} and demonstrate that connections between hub neurons have the highest transcriptional similarity, and connections between nonhubs having the least correlated gene expression, in striking agreement with previous results in the mesoscale mouse connectome \cite{Fulcher:2016ck}.
Our results hint that hub transcriptional similarity may be a conserved property of neuronal systems.

We demonstrate a relationship between neuronal connectivity and correlated gene expression in \textit{C. elegans}.


Combining a range of data sources, we demonstrate that the results cannot be attributed to the fact that hub neurons are interneurons, born prior to hatching, are more likely to be in the same network module, are mostly cholinergic, are mostly in the head, nor can it be attributed to the lineage similarity of hub neurons.
However, given the overlap between hub neurons in \emph{C. elegans} with the transcriptionally similar set of control interneurons, we are unable to discount the possibility that the high correlated gene expression between hub neurons is driven by that of the control interneurons.
Amongst the genes considered here, many of those with the greatest contribution to this signature are related to glutamate receptor, acetylcholine, and other neuronal communication related genes.
...
(iii) a combination of chemical and electrical synapses as an undirected network (representing unidirectional and reciprocal chemical connections equivalently) \cite{Towlson2013, Kim:2014bu, Pavlovic:2014gx};
...
Having characterized a robust relationship between hub connectivity and gene coexpression, we next investigated which specific genes were most important in driving the observed differences in correlated gene expression computed across all genes.
We developed a method to score genes according to their contribution to a given coexpression pattern (see \emph{Methods}).
Despite challenges with the incomplete binary expression measurements in a small proportion of the genome, we aimed to quantify any signals in the available data that may relate to the differences in correlated gene expression characterized above.
We characterize individual genes with the high scores, and attempted to summarize functional groups of genes as biological process categories of the gene ontology (GO) that were enriched in high scoring genes using overrepresentation analysis (ORA) \cite{Ashburner2000, Gillis2010}.
[[REMIND THE READER HOW MANY GENES WERE FED INTO THE ORA]]
% Pavlidis:2002ve
...
Biological process GO categories in \emph{C. elegans} are listed in Table~\ref{tab:enrichmentCON}.
...
Despite incomplete gene expression data that do not provide sufficient coverage to detect statistically significant effects, these highly scored GO categories provide a sensible summary of gene groups involved in neuronal connectivity.

To explore these results in more detail, we manually investigated individual high-scoring genes [[provide list as suppl with FDR p-values]].
Given that glutamate is one of the most popular neurotransmitters in the \textit{C. elegans} nervous system (26\% of neurons with known neurotransmitter type are glutamatergic [[CITATION??]]), and many high scoring genes are related to glutamate receptors (\emph{glr-1}, \emph{glr-2}, \emph{glr-4}, \emph{glr-5}, \emph{nmr-1}, \emph{nmr-2}).
Consistent with the importance of innexins in forming electrical synapses, the following innexin genes were all in the top X: \emph{unc-9}, \emph{unc-7}, \emph{inx-7}, \emph{inx-19}, \emph{inx-13} [[CITE]].
...
learning (\emph{casy-1}) \cite{Zallen1999, Garriga1999, Leung-Hagesteijn1992, Harris:2009kd, Ikeda2008} as well as two orthologs of Drosophila genes involved in determining cell polarity (\emph{vang-1} and \emph{prkl-1} \cite{Wu2006, Hoffmann2010}) are also amongst the highest scoring genes.
...
were also amongst the highest scoring genes for neuronal connectivity.
Both above mentioned adhesion molecules and transcription factors and potentially involved in facilitating signal transduction and communication.
Despite incomplete and noisy data, our results indicate that our data-driven gene scoring method is able to yield biologically interpretable genes.
...
For example, only one of the 948 genes annotated to the GO categories related to hub connectivity in mouse is present in our gene expression dataset (\emph{unc-32} is annotated to `ATP hydrolysis coupled proton transport').
% GO:0015991:
% GO categories were filtered using <<searchGOcategories.m>> function.
Although a direct test of the metabolic hypothesis is not possible from current data, we investigated whether other GO categories were overrepresented in pairs of connected hubs.
...
The top GO categories are listed in Table~\ref{tab:enrichmentRICH}.
Although no categories are statistically significant at a false discovery rate of 0.05, the list of top categories yields some interesting observations.
[[The same two glutamate receptor signaling categories]][[Not really clear what the two categories are anymore; either say explicitly here, or be clearer above, or do both]] implicated for connectivity above, were [[again at the top of the list]][[We no longer say that these were at the top of the connectivity list]] of categories related to hub connectivity.
Other top categories include a more general receptor signaling class, regulation of locomotion, and various metabolism and biosynthesis related processes.
...
In order to investigate what specific genes are implicated in the increased coexpression for connections involving hubs, we again look at the list of genes used for ORA. 
In addition to glutamate receptor (\emph{glr-5}, \emph{nmr-1}, \emph{nmr-2}, \emph{glr-1}, \emph{glr-2}, \emph{grld-1})
and acetylcholine related genes (\emph{ace-2}, \emph{cho-1}, \emph{unc-17}, \emph{deg-3}), we again find a high number of adhesion molecules (\emph{cam-1}, \emph{rig-1}, \emph{rig-6}, \emph{unc-6}, \emph{grld-1}, \emph{dbl-1}, \emph{ncam-1})
...
Thus, our gene scoring method was able to yield some interpretable insights into the types of genes that may be contributing to different types of connectivity signatures in \emph{C. elegans}.
Future work leveraging more comprehensive expression data could be used to more comprehensively understand the relationship between network topology, neurotransmitter signaling, and other gene-regulated functions.
...
We find that connections between hub neurons have the highest transcriptional similarity, and connections between nonhubs having the least correlated gene expression.
This result, in a specialized neuronal nervous system, is in striking agreement with previous results in the mesoscale mouse connectome \cite{Fulcher:2016ck}, hinting that hub transcriptional similarity may be a conserved property of neuronal systems.
Combining a range of data sources, we demonstrate that the results cannot be attributed to the fact that hub neurons are interneurons, born prior to hatching, are more likely to be in the same network module, are mostly cholinergic, are mostly in the head, nor can it be attributed to the lineage similarity of hub neurons.
However, given the overlap between hub neurons in \emph{C. elegans} with the transcriptionally similar set of control interneurons, we are unable to discount the possibility that the high correlated gene expression between hub neurons is driven by that of the control interneurons.
Amongst the genes considered here, many of those with the greatest contribution to this signature are related to glutamate receptor, acetylcholine, and other neuronal communication related genes.
The methods we develop here for quantifying correlation gene expression and scoring genes on their contribution to it yield biologically interpretable results on incomplete, binary gene expression data.
...
\textbf{MAIN EFFECT}
% COMPARE TO MOUSE
Compared to our previous work in the mesoscale mouse connectome \cite{Fulcher:2016ck}, the present findings in a fully mapped neuronal connectome show both similarities and differences.
...
We attempted to address this by looking at potential confounds, to characterize whether high coexpression associated with rich connections was related to aspects like interneuron type, neurotransmitter system, neuronal birth time, neuronal lineage, anatomical location, modular organisation of the connectome as well as functional class.
We were able to reject influences of all except one potential driving factors. 
Keeping in mind the highly specialised nature of the neurons in \textit{C. elegans} connectome it is impossible to separate the effect of topological organisation and a functional class as the highest degree neurons also happen to belong to the functional class of command interneurons. 
