...
\begin{figure}[h]
  ...
  \textbf{A} CGE values, $r_\phi$, plotted as a function of Euclidean separation distance for all pairs of neurons within the head (gray dots), with a fitted exponential trend shown in black, $f(x) = A\exp(-\lambda x) + B$.
    ...
    This indicates that the trend is not a bulk, isotropic effect in the brain, but may instead be driven primarily by a small number of neuron pairs with high $r_\phi$ at short distances ($\lessapprox 25\mu$m), indicated with a circle in \textbf{A}.
    }
