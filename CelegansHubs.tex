...
[[AA: Towlson directly separated nodes into hubs/non-hubs, but Varier first said that "Early stages are also crucial for the formation of highly connected neurons (hubs)" so I'd say they are the primary source??? OR could cite both without saying "first"]]\\

The formation of neurons in \textit{C. elegans} nervous system is separated into two distinct time periods: before hatching (neuron birth time $< 550$ min) and after hatching (neuron birth time $> 1200$ min) with an around 700min gap between them when no neurones are formed.  
...
To test this hypothesis we separated all neurons that were formed before hatching into two groups based on their degree: early-born hub neurons (birth time$<1000$ min, degree$>41$) and early-born non-hub neurons (birth time$<1000$ min, degree$\leq41$) and compared coexpression between those groups (Fig \ref{fig:BirthTimeHubs}).

Even among early born neurons, connections between hubs have significantly increased coexpression compared to connections between non-hubs from the same time period (add p value, ranksum value). 
Therefore, we confirm that increased coexpression between hub neurons can not be explained by their similarity in birth time and that hubs exhibit a unique transcriptional pattern.

\begin{figure}[t]
  \centering
    \includegraphics[width=0.5\textwidth]{BirthTimeHubs.eps}
...
\textbf{B} Yule's Q, and \textbf{C} Jaccard correlation measures both exhibit a strong systematic bias.
