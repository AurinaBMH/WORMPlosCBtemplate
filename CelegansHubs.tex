...
\begin{figure}[h]
\centering
   \includegraphics[width=1\textwidth]{TypeDegree2.pdf}
 \caption{
...
[[TODO-AA: Add labels A and B, can use the LabelCurrentAxes function]]
...
Given that hubs exhibit significantly higher coexpression compared to other interneurons ($P < 10^{-34}$, Wilcoxon rank sum test, data not shown),
[[TODO-AA: what does it mean that hubs exhibit higher coexpression? Hubs are regions, coexpression is pairs. This should be a clear statement of a result, not a ``given that X, ...'' if X is a new result.
Also, Fig.7B looks very similar to this!]].
To test whether the observed increase in coexpression for rich links is not determined by the fact that the majority of hub neurons belong to the same anatomical neuron type, we selected a subset of interneurons that most closely resembles hub interneurons in terms of their anatomical properties, i.e., that originate and terminate in approximately the same locations as hub interneurons.
...
Four modules were extracted, with 1 hub in module 1 (contains 40 neurons), 4 hubs in module 2 (96 neurons), 11 hubs in module 3 (111 neurons), and no hubs in module 4 (32 neurons).
As expected, gene coexpression was significantly increased for pairs of neurons with modules (2\,080 intramodular neuron pairs) relative to connections between modules (817 intermodular neuron pairs) ($p \approx 0$, Wilcoxon rank sum test).
% CoexpressionDistributionsModules(C,G,'intraInter',false,'consensus');
...
Similar results were found using a previously reported nine-module partition of neurons derived from an Erd\"os-R\'enyi Mixture Model ($p = 4 \times 10^{-208}$, Wilcoxon rank sum test) \cite{Pavlovic2014}.
% CoexpressionDistributionsModules(C,G,'intraInter',false,'ERMM');
...
Focusing on pairs of neurons within the same module, we found that coexpression was higher for pairs of hubs than pairs of nonhubs, as shown in Fig.~\ref{fig:OtherInfluences}C ($p = 8.5\times 10^{-17}$, Wilcoxon rank sum test).
% CoexpressionDistributionsModules(C,G,'hubIntra',false,'consensus');
...
Thus, despite an increase in coexpression between pairs of neurons within the same module, this effect does not explain the increase in hub-hub coexpression.
...
[[TODO: PAIRWISE RATHER THAN CONNECTED FOR CONSISTENCY??]]
...
Moreover, tissue distance (resembling lineage distance on a cellular scale) was found to correlate with gene expression divergence in both human and mouse \cite{Cui2007}, suggesting a possible relationship between lineage distance and gene coexpression in a micro-scale organism [[??]].
...
However, as shown in Fig.~\ref{fig:OtherInfluences}E, there is no significant difference in lineage distance between hub (rich connections) and non-hub neurons (non-rich connections) ($P = 0.0786$, Wilcoxon rank sum test) [[Could do as pairs rather than connected...?]].
...
[[IS IT COEXPRESSION betheen all pairs or connected pairs]]
...
We developed a method to assign a gene coexpression contribution score to each gene that quantifies the contribution of each gene to a given coexpression value for a pair of neurons. These scores were then used to perform an enrichment analysis using gene ontology (GO) categories for biological processes \cite{Ashburner2000, Gillis2010} (see \emph{Methods}).
...
In our analysis, however, no categories remained significant after the corrections for a false discovery rate ($FDR < 0.1$), therefore we present a list of top GO categories with corresponding uncorrected p values in Table~\ref{enrichmentCON}.
The most influential GO categories here are related to glutamate receptor signaling: `ionotropic glutamate receptor signaling pathway' ($p_\mathrm{uncorr} = 0.0005$) and `glutamate receptor signaling pathway' ($p_\mathrm{uncorr} = 0.004$) as well as 
cell surface receptor signaling ($p_\mathrm{uncorr} = 0.005$), `ion transport' ($p_\mathrm{uncorr} = 0.006$) and `ion transmembrane transport' ($p_\mathrm{uncorr} = 0.008$). 
Other categories are involved in the regulation of growth rate and several catabolic processes. 
...
Entries have been sorted (descending) by uncorrected $P$-value (uncorrected).}
\label{enrichmentCON}
\begin{tabular}{llll}
\hline
\textbf{GOcategory} 	& \textbf{Description}                                     & \textbf{\begin{tabular}[c]{@{}c@{}}No. of \\ genes\end{tabular}} & \textbf{P value} \\ \hline
GO:0035235 & ionotropic glutamate receptor signaling pathway & 7            & 0.0005  \\
GO:0007215 & glutamate receptor signaling pathway            & 9            & 0.0041   \\
GO:0007166 & cell surface receptor signaling pathway         & 37           & 0.0048   \\
GO:0006811 & ion transport                                   & 71           & 0.0058   \\
GO:0034220 & ion transmembrane transport                     & 57           & 0.0079   \\
GO:0055085 & transmembrane transport                         & 67           & 0.0129 \\
GO:1901575 & organic substance catabolic process             & 33           & 0.0310 \\
GO:0040009 & regulation of growth rate                       & 7            & 0.0417 \\
GO:0040010 & positive regulation of growth rate              & 7            & 0.0417 \\
GO:0030163 & protein catabolic process                       & 27           & 0.0428 \\
GO:0009056 & catabolic process                               & 35           & 0.0493 \\
GO:0009057 & macromolecule catabolic process                 & 28           & 0.0548 \\
GO:0071495 & cellular response to endogenous stimulus        & 14           & 0.0575 \\
GO:0045927 & positive regulation of growth                   & 29           & 0.0689 \\
GO:0050830 & defense response to Gram-positive bacterium     & 5            & 0.0695 \\\hline
\end{tabular}
\end{table}
...
\hline
\textbf{GOcategory} & \textbf{Description}                                                                                                        & \textbf{\begin{tabular}[c]{@{}c@{}}No. of \\ genes\end{tabular}}  & \textbf{P value}    \\\hline
GO:0007215 & glutamate receptor signaling pathway                                                                               & 6            & 0.0009  \\
GO:0035235 & ionotropic glutamate receptor signaling pathway                                                                    & 6            & 0.0009  \\
GO:0007166 & cell surface receptor signaling pathway                                                                            & 20           & 0.0035   \\
GO:0009891 & positive regulation of biosynthetic process                                                                        & 11           & 0.0063   \\
GO:0031328 & \begin{tabular}[c]{@{}l@{}}positive regulation of cellular \\ biosynthetic process\end{tabular}                    & 11           & 0.0063   \\
GO:0045935 & \begin{tabular}[c]{@{}l@{}}positive regulation of nucleobase-containing \\ compound metabolic process\end{tabular} & 11           & 0.0063   \\
GO:0051173 & \begin{tabular}[c]{@{}l@{}}positive regulation of nitrogen compound \\ metabolic process\end{tabular}              & 11           & 0.0063  \\
GO:0031325 & \begin{tabular}[c]{@{}l@{}}positive regulation of cellular \\ metabolic process\end{tabular}                       & 12           & 0.0109 \\
GO:0040012 & regulation of locomotion                                                                                           & 24           & 0.0156 \\
GO:0010557 & \begin{tabular}[c]{@{}l@{}}positive regulation of macromolecule \\ biosynthetic process\end{tabular}               & 10           & 0.0212 \\
GO:0045893 & \begin{tabular}[c]{@{}l@{}}positive regulation of transcription, \\ DNA-templated\end{tabular}                     & 10           & 0.0212 \\
GO:0045944 & \begin{tabular}[c]{@{}l@{}}positive regulation of transcription from \\ RNA polymerase II promoter\end{tabular}    & 10           & 0.0212 \\
GO:0051254 & positive regulation of RNA metabolic process                                                                       & 10           & 0.0212 \\
GO:1902680 & positive regulation of RNA biosynthetic process                                                                    & 10           & 0.0212 \\
GO:1903508 & \begin{tabular}[c]{@{}l@{}}positive regulation of nucleic acid-templated \\ transcription\end{tabular}             & 10           & 0.0212\\ \hline
\end{tabular}
\end{table}
