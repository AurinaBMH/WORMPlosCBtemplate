...
For example, we aim to compare the contributions of each gene to increases in coexpression observed for (i) connected vs unconnected pairs of neurons, (ii) electrical versus chemical connections, or (iii) rich and feeder vs peripheral edges.
...
168 genes for the hub connectivity analysis.

%Given the reduced number of genes remaining for analysis following the quality control (threshold on $n \geq 10$), 
We performed an over-representation analysis (ORA) using the genes that contribute most to a given connectivity pattern.
