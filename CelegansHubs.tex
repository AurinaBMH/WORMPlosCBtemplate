...
For the same reasons as noted above, bilateral pairs of neurons were excluded from all scoring procedures.
% as they are also excluded from the gene expression analysis.
To ensure that each gene contributes a meaningful score, we set a threshold on the minimum number of possible matches, $n \geq 10$ in both analyses.
Additional details on gene scoring for specific analyses performed here are in the Supplementary Information.
