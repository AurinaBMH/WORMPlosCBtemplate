...
The formation of neurons in \textit{C. elegans} nervous system can be separated into two distinct time periods: before hatching (neuron birth time $< 550$ min) and after hatching (neuron birth time $> 1200$ min) with an around 700min gap between them when no neuron formation is happening.  
...
To test this hypothesis we separated all neurons from the first formation period into two groups based on their degree: early-born hub neurons (birth time$<1000$ min, degree$>41$) and early-born non-hub neurons (birth time$<1000$ min, degree$\leq41$) and compared coexpression between those groups (Fig \ref{fig:BirthTimeHubs}).

Even among early born neurons, connections between hubs have significantly increased coexpression compared to connections between non0hubs from the same time period. 
Therefore, we confirm that increased coexpression in hub neurons can not be explained by their similarity in birth time and that hubs exhibit a unique transcriptional pattern.
