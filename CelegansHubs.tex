...
(ii) \emph{mismatched} $g^{(a)}_i = 1$ and $g^{(a)}_j = 0$ or $g^{(a)}_i = 0$ and $g^{(a)}_j = 1$;
...
In particular, we were interested in quantifying increases in a gene being expressed in both regions of a given class of inter-region pairs, which we computed as a cumulative binomial probability of obtaining at least $m$ matches ($g^{(a)}_i = 1$ and $g^{(a)}_j = 1$) being observed empirically:
\begin{eqnarray}
	\label{eq:CBinomialProbability}
     P(X > m) = 1 - \sum_{i=0}^{m}\binom{n}{i} p^{i}(1-p^{n-i}),
\end{eqnarray}
where $p$ is the probability of the given connection class (as $n_\mathrm{class}/M$ for $n_\mathrm{class}$ connections out of $M$ possible pairs), and $n$ is the maximum number of possible matches.
...
When comparing links involving hubs compared to those between nonhubs, $p$ is the proportion of connections that involve hubs.
...
\texttt{Generic\_worm\_noParents.an.txt.gz} downloaded using a build-in option within ErmineJ software on February 10 2017.
...
A schematic overview of our analysis is shown in Fig.~\ref{fig:SchematicRepresentation}.
We show binary expression data, the binary topology of the directed synaptic connectome, and our aim to investigate relationships between the pairwise patterns of synaptic connectivity and gene coexpression.
