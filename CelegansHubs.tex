...
In Fig.~\ref{fig:topology_rich}B, we plot the mean distance between all pairs of connected hubs at each degree threshold, $k$, again finding an increase with $k$ through to the topological rich-club regime.
Given that the connection probability decreases with distance (Fig.~\ref{supp:})
(exponentially in mouse \cite{Fulcher:2016ck})
...
A decreasing relationship is evident up to a separation distance of approximately 0.2\,mm.
The slight increase at large distances ($\approx 1$\,mm) may be due to the importance of head-tail connections relative to connections between the body and the head or tail.
\textbf{B}
Pearson gene coexpression, $r_\phi$, is plotted as a function of separation distance for all pairs of neurons
}
\end{figure}
