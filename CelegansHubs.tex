...
\begin{figure}[!h]
   \centering
    \includegraphics[width=0.7\textwidth]{RecUnidirUnconFINAL}
...
%%
\subsection*{Rich club organisation of the connectome}
\begin{figure}[!h]
   \centering
    \includegraphics[width=0.7\textwidth]{RCcurve}
 \caption{{\bf Rich-club organisation of the connectome.} \\
Top: normalized rich club coefficient, $\Phi_{random}$ as a function of the degree, \textit{k}, at which hubs (regions with degree $\geq \textit{k}$) are defined. 
$\Phi_{random}>1$ indicates that hubs are more densely interconnected among each other than expected by chance. 
Red circles indicate values of $\Phi_{random}$ that are significantly higher than an ensemble of 1000 null networks (permutation test; $P<0.05$). \\
Bottom: degree distribution of C. elegans connectome for chemical synapses. }
 \label{RCcoef}
 \end{figure}
 
%%  
\subsection*{Coexpression and hub connectivity}
\begin{itemize}
...
At each level of node degree, $k$ (x-axis in Figure 5), we labeled each neuron as either a hub (nodes with degree $\geq k$) or a nonhub (otherwise), and then labeled each connection as rich (hub - hub), feeder (nonhub - hub or hub - nonhub), or peripheral (nonhub- nonhub). Across the topological rich club regime, mean gene coexpression is significantly higher for rich connections (red in Figure 6) and increases slightly for feeder links from $k=52$.
 ...
 \begin{figure}[!h]
 \centering
    \includegraphics[width=1\textwidth]{RichFeederPeripheral_k}
...
Coexpression ($\phi$) for different types of network connections, where hubs are neurons with degree $k \geq 42$. P-values are from pairwise comparisons using Welch’s t test. Should probably NOT include this plot}
 \label{ViolinPlots}
 \end{figure}
