...
The nervous system of \emph{C. elegans} is modular (\cite{Bassett2010a, Pavlovic2014, Kim2014a } on previous work?]], containing densely connected groups of neurons that are relatively sparsely connected to other neurons.
Could gene expression differences between modules drive the increased gene coexpression of hubs?
To test this possibility, we used the Louvain community detection algorithm (see \textit{Methods}) to extract modules from the binary C. elegans synaptic connectome, which we label M1--M4, shown in Fig.~\ref{fig:modules}.
