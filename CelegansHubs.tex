...
We sought to determine whether certain sets of genes drive specific patterns of coupled expression between particular classes of neuron pairs (e.g., connected \emph{vs} unconnected pairs of neurons, pairs of neurons connected via synapses \textit{vs} gap junctions, or between rich/feeder/peripheral connections) using gene enrichment analysis.
...
Note that here we excluded bilateral pairs of neurons from the scoring.
...
To ensure that each gene contributes a meaningful score, we set a threshold on the maximum number of matches, $n \geq 5$.
[[AA-UPDATE: A total of 577 genes out of 948 satisfied this criterion for connected/unconnected analysis; 483 out of 948 genes satisfied this criterion for hub and synapse type connectivity analysis.]]
...
We included ontology terms containing between 5 and 100 annotations, using full resampling with $10^{5}$ iterations [[CHECK WE ACTUALLY DID THIS MANY RUNS]].
...
For connections across the body, ranging up to $\approx 1$\,mm, a near-exponential trend is exhibited, mirroring results in other species and across longer length scales \cite{Wang:2016gg, Fulcher:2016ck}.
...
Attempting to correct the trend in this way produces artifactual reductions in the coexpression of many neuron pairs (shown in Fig.~\ref{fig:Supp}).
...
Thus, we chose to analyze spatially-uncorrected expression data through this work.
...
\begin{figure}[h]
  ...
  An exponential relationship, $f(x) = A\exp(-\lambda x) + B$, is fitted in \textbf{A} and \textbf{B}, revealing a slight decreasing trend in $r_\phi$ with distance, as in macroscopic mammalian brains \cite{Fulcher:2016ck, Krienen:2016eq}.
  }
...
Other top GO categories include cell surface receptor signaling ($p_\mathrm{uncorr} = 0.002$), memory ($p_\mathrm{uncorr} = 0.005$), `ion transport' ($p_\mathrm{uncorr} = 0.01$) and `regulation of growth' ($p_\mathrm{uncorr} = 0.01$).
...
\begin{figure}[h]
  ...
  An exponential trend fitted to the data is shown in black, $f(x) = A\exp(-\lambda x) + B$.
    The trend is not a bulk effect that holds in general across the brain, but is driven primarily by a small number of neuron pairs with high $r_\phi$ at short distances ($\approxless 25\mu$m).
