...
Given that no categories remained significant after the corrections for a false discovery rate ($FDR < 0.1$), we present a list of top GO categories with corresponding uncorrected p values.
These GO categories are related to glutamate receptor signaling: `ionotropic glutamate receptor signaling pathway' ($p_\mathrm{uncorr} = 0.0005$) and `glutamate receptor signaling pathway' ($p_\mathrm{uncorr} = 0.004$) as well as 
...
Other categories are related to the regulation of growth rate and various catabolic processes. 
...
The same two glutamate receptor signaling categories found above for the connectivity were, again, among the top categories, however as before no GO categories remained significant after FDR<0.1 correction.
In addition, another receptor signaling category 'cell surface receptor signaling pathway'  ($p_\mathrm{uncorr} = 0.004$) was also among the most important GO categories enriched in connections involving hubs compared to other connections.
Other top categories are related to regulation of locomotion ($p_\mathrm{uncorr} = 0.015$) and various metabolism and biosynthesis related processes including 
NAME THEM

 `cell surface receptor signaling pathway' ($p_\mathrm{uncorr} = 0.004$), `cell-cell signaling' ($p_\mathrm{uncorr} = 0.004$), `chemical synaptic transmission' ($p_\mathrm{uncorr} = 0.005$), `synaptic signaling' ($p_\mathrm{uncorr} = 0.005$).
