...
Dense interconnectivity of these hubs is known as rich-club organization.
...
Our results suggest that neural hubs possess a distinctive transcriptional signature, preserved across scales and species, that is related to the involvement of hubs in regulating the higher-order behaviors of a given organism.
...
Here we find that connections between hub neurons are the most similar in their gene expression.
We show that this effect cannot be explained by a range of neuron properties such as neuron type, position, birth time, lineage or neurotransmitter system. Instead, we show that it is linked to the functional class of command interneurons and is driven by coupled expression of genes regulating glutamate, acetylcholine and neuronal communication related genes.
...
They found that transcriptional coupling, across all genes, was stronger for connected compared to unconnected brain regions and that, in general, this coupling decayed sharply with increasing anatomical distance between brain regions.
In contrast to this general trend, connected pairs of hubs (i.e., the ``rich club'' of the brain) showed the highest levels of transcriptional coupling (compared to hub-nonhub and nonhub pairs), despite these regions being separated by larger distances, on average.
...
Paralleling findings in humans and mouse, we show that connections between hub neurons in the \textit{C. elegans} connectome are most similar in their gene expression, and that this effect is driven by genes regulating glutamate and acetylcholine function and neuronal communication.
Furthermore, we demonstrate that this effect is not explained by factors such as neuronal birth time, lineage, neurotransmitter system or spatial position but is rather related to the functional specialization of hub neurons, and may be related to their role in mediating higher order behaviours of the organism.
...
The first method involved applying the Louvain community detection algorithm \cite{Blondel:2008do} using the \texttt{community\_louvain} function implemented in the Brain Connectivity Toolbox \cite{Rubinov:2010jd}.
...
Our primary aim in this work is to understand how pairwise patterns of neuronal connectivity (i.e., Fig.~\ref{fig:SchematicRepresentation}A) relate to coupled expression across 948 genes between pairs of neurons (i.e., pairs of rows of Fig.~\ref{fig:SchematicRepresentation}C).
...
Note that our basic findings replicated regardless of whether we use $r_\phi$ or this new measure to quantify coupled expression.
...
\bibliography{library_ben,library_aurina2}
