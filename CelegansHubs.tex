...
Other top GO categories include memory ($p_\mathrm{uncorrected} = 0.005$), `ion transport' ($p_\mathrm{uncorrected} = 0.007$), `nonassociative learning' ($p_\mathrm{uncorrected} = 0.01$), `locomotory behavior' ($p = 0.02$), and `protein metabolic process' ($p_\mathrm{uncorrected} = 0.02$).
...
However, in our \emph{C. elegans} gene expression dataset, only one of our 948 genes was annotated to the GO categories related to hub connectivity in mouse (\emph{unc-32} is annotated to ATP hydrolysis coupled proton transport, GO:0015991).
...
Other top categories are also similar to the connectivity signature, including `nonassociative learning' ($p_\mathrm{uncorrected} = 0.01$), and `locomotory behavior' ($p_\mathrm{uncorrected} = 0.02$).
Metabolic processes were also among the most enriched categories, including `neurotransmitter metabolic process' ($p_\mathrm{uncorrected} = 0.04$), and `positive regulation of nucleobase-containing compound metabolic process' ($p_\mathrm{uncorrected} = 0.05$).
...
Although our data do not support strong confidence in enrichment, the results fit broadly with expectations.
For example, the majority of hubs are command interneurons, which has an established function of controlling forward and backward locomotion.
 % enrichment of genes related to locomotory behavior aligns with expectations.
...
The significant enrichment of glutamate receptor signaling for both connectivity, and hub connectivity was somewhat surprising, as only 26\% of neurons with known neurotransmitter type are glutamatergic (whereas 55\% are cholinergic), and only 2 of the 13 hubs ($k > 41$) are glutamatergic; the remaining 11 are cholinergic (Fig.~\ref{fig:S_feedInOutNodes}C).
...
Both metabolic and glutamate signaling related categories were expected for feed-in links. 
It is worth keeping in mind that only one acetylcholine-related GO category (GO:0007271) was represented in the current gene expression data while it at all lack the representation of any glutamate release categories (there is no GO category with at least 5 genes available). 
...
\centering
\caption{
Biological processes GO categories showing increased gene coexpression for rich links compared to all other links.
% (feed-in, feed-out, peripheral)
Categories that remain significant after false discovery rate ($FDR < 0.05$) correction are shown boldface.
}
