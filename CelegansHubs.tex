...
Vector pairwise correlations were calculated using different correlation measures and an average of 1\,000 runs was taken to summarize the behavior of each measure.
...
(ii) sensory (68 neurons, green),
(iii) motor (108 neurons, blue), or
...
\textbf{A} \emph{Upper}: The number of connections involving interneurons (orange), sensory neurons (green), and motor neurons (blue) across degree threshold, $k$ [[TODO: Aurina, is it correct that sensory is green and motor is blue?]].
...
The nervous system of \emph{C. elegans} is modular [[Refs?]], containing densely connected groups of neurons that are relatively sparsely connected to other neurons.
Could the modular membership of hub neurons drive their increased gene coexpression patterns?
We used the Louvain community detection algorithm (see \textit{Methods}) to extract four modules (labeled M1, M2, M3, M4) from the binary C. elegans synaptic connectome, shown in Fig.~\ref{fig:modules}.
The algorithm picks up three comparably sized modules (M1-M3), each containing diverse neuron types, as well as a smaller fourth module (M4), which contained 13 motor neurons.
...
\textbf{A} Connectome organized by module, with all connections labeled by type: rich (red), feed-in (yellow), feed-out (orange), and peripheral (blue).
\textbf{B} Distributions of coexpression within and between modules reveal no mean difference.
\textbf{C} Not sure what this plot is of. I assume rich links within modules?
