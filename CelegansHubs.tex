...
After confirming the rich-club connectivity of the \emph{C. elegans} connectome, we show that connections between hub neurons have the highest transcriptional similarity, and connections between nonhubs having the lowest transcriptional similarity, consistent with previous results at the mesoscale in mouse \cite{Fulcher:2016ck}.
...
Despite incomplete gene expression data that do not provide sufficient coverage to detect statistically significant effects, these highly scored GO categories show a sensible summary of genes most involved in neuronal connectivity.

To explore these results in more detail, we manually investigated individual high-scoring genes submitted to ORA analysis (see supplementary data GeneList.xlsx).
...
as well as two orthologs of \emph{Drosophila melanogaster} (\emph{vang-1} and \emph{prkl-1} \cite{Wu2006, Hoffmann2010}) and human (\emph{prkl-1}) genes involved in determining cell polarity are also amongst the selected 20\% of the highest scoring genes. 
...
Despite incomplete and noisy data, these results indicate that our data-driven gene scoring method is able to yield sensible, biologically relevant insights into the genetic basis of neuronal connectivity in \textit{C. elegans} connectome. 
...
Although no categories are statistically significant at a false discovery rate of 0.05, the list of top categories includes both 'glutamate receptor signaling pathway' as well as more general 'cell surface receptor signaling pathway' in addition to several ion transport related gene groups ('ion transport', 'ion transmembrane transport', 'transmembrane transport').  
...
Many spatial patterns in the data are consistent with that of macroscopic mammalian neuronal systems, including the decreasing spatial dependence of connection probability for pairs of neurons within the head, tail, and body, and a decreasing spatial trend in many patterns of correlated gene expression.
...
It is reasonable to expect that the rules of neural organization may differ from the scale of individual neurons to the scale of macroscopic brain regions that each containing millions of individual neurons.
...
However, unlike mammalian brains, where all neurons are confined to a spatially contiguous organ, neurons are distributed throughout \emph{C. elegans}, including a dense cluster of neurons in the head and in the tail.
We report qualitatively similar spatial dependence of connection probability with separation distance in the specialized neuronal nervous system of \emph{C. elegans} for many classes of connections (including those within the head, body, and tail), indicating that this may be a generic property of evolved neuronal systems that must balance the energetic cost of long-range connections with their functional benefit \cite{vandenHeuvel:2012kh, Kim:2014bu}.
...
Our results highlight the importance of treating the brain as a spatially embedded object, as many seemingly non-trivial properties of brain organization may be well approximated by simple, isotropic spatial rules \cite{Henderson:2014fg, Roberts2016, Horvat:2016ia, Bassett:2010hf, Chen:2006ie}.
...
% COMPARE TO MOUSE
The correlated gene expression patterns in \emph{C. elegans} show many surprising similarities to previous work in the mesoscale mouse connectome \cite{Fulcher:2016ck}, despite:
...
and involving different gene expression annotation data (comprehensive in situ hybridization expression data across $\sim20\,000$ genes in mouse versus literature curated annotations across $\sim 1000$ genes in \emph{C. elegans}).
...
Additionally, we can not rule out the possibility that gene annotations have been influenced by the nature of a curated dataset.
...
While the concerns regarding biased gene annotations might be valid, we do not have any empirical evidence to support that at the moment, therefore only further development in gene expression data could shed some light in this regard.
...
Although gene enrichment analyses did not have enough power to detect significant effects, top GO categories point us into the right direction given biologically relevant categories related to neuronal connectivity, neurotransmitters, and metabolism emerge at the top.
...
Given \textit{Drosdophila} (\textit{vang-1}, \textit{prkl-1}) and human (\textit{prkl-1}) orthologs related to cell polarity were implemented in neuronal connectivity of the \textit{C. elegans}, methods outlined in this paper could be potentially implemented in making insights into targets for further research in other species.
[[Even with limited data, indicates ability of our framework to make predictions of genes, that have plausibility given the role of these genes in neuronal functioning in other species, that may in future be validated directly in {\it C. elegans}]]
...
% \textbf{LIMITATIONS}
Compared to continuous \emph{in situ} hybridization measurements of the expression of $>17\,000$ genes in the mouse brain \cite{Lein:2007jn} or microarray measurements of $>20\,000$ genes in the human brain \cite{Hawrylycz:2012ky}, which permit more detailed analysis \cite{Fulcher:2016ck, Ji:2014jw, Fakhry:2015kl, French2011, Vertes2016a, Parkes:2017dn}, working with \emph{C. elegans} gene expression data is challenging due to its low coverage ($<5$\% coverage of the full worm genome), binary indications of expression, and incompleteness (an inability to distinguish missing data from lack of expression).
...
[[Refs?: \cite{Lein:2007jn, Shen:2012ua, Tasic:2016jp}]]
...
This data has different qualifiers, therefore some arbitrary choices need to be made in order to keep the balance between sensitivity and specificity. 
