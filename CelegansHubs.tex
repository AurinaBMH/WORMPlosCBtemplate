...
The axonal wiring between elements of nervous systems displays an incredible complexity, with the web of connections between neural elements providing insights into how cognitive function might have evolved under physical and energetic constraints.
...
This allows very different species, measurements, and scales of neuronal networks to be represented in a consistent and unified way.
...
As well as showing a modular network structure, that can often be interpreted in terms of distinctive functional brain networks, connectomes have been consistently shown to exhibit a non-uniform distribution of connectivity across the network, containing a small number of highly connected `hub' nodes.
Hubs have also been shown to be densely interconnected with each other, forming an integrated `rich club' that is costly to develop (containing stronger, longer-distance projections) but delivers functional benefits by facilitating efficient integration of information between more specialized processing modules \cite{VandenHeuvel2012}.
...
The recent availability of spatial gene expression and other molecular-level data across a range of neural systems has allowed structural connectome properties to be related across spatial scales to molecular information \cite{vandenHeuvel:2017ex}.
...
Later work, utilizing detailed quantitative gene expression data in mouse from the Allen Mouse Brain Atlas \cite{Lein:2007jn} revealed a relationship between gene expression and connectivity in mouse \cite{Fakhry:2015kl, Ji:2014jw}, with pairs of brain regions with similar gene expression patterns also displaying similar connectivity profiles \cite{French2011}.
% Fakhry:2015dg
% that connected pairs of brain regions display more similar gene expression profiles \cite{French2011},
Looking further into different types of connections in mouse, Fulcher and Fornito showed that pairs of connected hubs have the most similar gene expression profiles, and that hub connectivity is associated with the correlated expression of genes related to oxidative energy metabolism \cite{Fulcher:2016ck}.
Similar findings have been reported at the regional level in the human cortex, with integrative hub regions (with high inter-modular degree and long connection distance) being associated with expression of genes regulating oxidative metabolism and mitochondria \cite{Vertes2016a}, consistent with their high metabolic cost \cite{Collin:2014kq, Tomasi:2013kl}.
...
Spatial information about neurons was obtained for 277 neurons (including VC6 which makes connections only through neuro-muscular junctions) [[TODO: Aurina, why mention this? AA - this neuron doesn't have obvious connections, therefore not included in the connectome, but we have coordinates for it, otherwise, numbers do not add]]) as two dimensional spatial coordinates from \url{www.biological-networks.org/?page_id=25}.
Coordinates for three neurons (AIBL, AIYL, SMDVL) were missing in this dataset, and were reconstructed based on the positions of the contralateral neurons (AIBR, AIYR, SMDVR) by assigning identical coordinates for the contralateral neurons in these three pairs according to Varier \cite{Varier2011} [[TODO: Explain the precedent for doing this. AA: Varier 2011 does not explain why they do it.]].
...
Note that some head neurons (12: ALA, AVFL, AVFR, AVG, RIFL, RIFR, RIGL, RIGR, SABD, SABVL, SABVR, SMDVL) were not labeled as head in this file, and were thus manually corrected by matching the assignment against the annotations provided at WormAtlas (http://www.wormatlas.org/).
...
To identify the optimal modular assignment, the algorithm was repeated 1\,000 times resulting in a thousand community affiliation vectors.
Like all methods that optimise the measure of modularity, Louvain community detection algorithm is constrained within the intrinsic limits of modularity maximization, therefore unique partitions differ in their node assignments. 
In order to cope with those stochastic fluctuations, we use the consensus clustering approach \cite{Lancichinetti2012a} which produces a stable partition using information from multiple runs of the modularity detection algorithm. 
...
We then applied a consensus clustering approach to this agreement matrix, as described by Lancichinetti and Fortunato \cite{Lancichinetti2012a}.
...
Expression annotations are made directly (when annotated genes are directly expressed in certain neurons), and indirectly to groups or broader classes of neurons (e.g., `head', `interneuron', etc.) (when genes are expressed in some neurons for a certain hierarchical category of cells), with different qualifiers defining levels of certainty: `certain' (gene A was observed to be expressed in cell Y), `enriched' (gene A has been found enriched in a certain dataset through microarray, RNAseq or proteomic analysis), `partial' (gene A was observed to be expressed in some cells of a group of cells that include Y), and `blank' (data from studies before 2005) and ‘uncertain’ (gene A was sometimes observed to be expressed in cell Y or gene A was observed to be expressed in a cell that could be Y).
...
In this particular dataset we are unable to distinguish relative gene expression levels between neurons as well as the following two cases: (i) ``gene is not expressed'' and (ii) ``there is no information on whether gene is expressed'', which are both represented as a ``0''.\\

The analyses presented here are of expression data that is annotated directly to individual neurons with one of the following qualifiers: `certain', `enriched', `partial', or `blank'. We chose to not include indirectly annotated genes as well as genes qualified as 'uncertain' in order to as much as possible avoid false-positive gene assignments.  
...
A total of 92 anatomically distinct neuron classes are bilateral, that is, they contain both left [[TODO: left what? Is there a correct terminology?AA: haven't seen a name for that]] and right counterparts (e.g., AVAL/AVAR).
...
Considering that the effect was not pronounced and no standard correction could be applied, we use original uncorrected gene coexpression data for all the further analyses. 
% Although an exponential form fits the mean coexpression as a function of separation distance well, the trend is driven by spatially close pairs of neurons with high coexpression, such that taking residuals after subtracting the trend line (as \cite{Fulcher:2016ck} for mouse data) leaves artefacts.
[[AA: using data without L/R pairs, exp fit fails]].
...
\textbf{B} Distributions of gene coexpression, $r_\phi$, for intra-modular connections (black) and inter-modular connections (gray), shown as violin plots with the median shown as a horizontal bar.
The number of connections of each type is shown in parentheses.
\textbf{C} Distributions of gene coexpression, $r_\phi$, for intra-modular rich (red) and non-rich (blue) connections, shown as violin plots with the median shown as a horizontal bar.
The number of connections of each type is shown in parentheses.
\textbf{D} Distributions of gene coexpression, $r_\phi$, for early born hubs (rich links, red) and non-hubs (non-rich links, blue) shown as violin plots with the median shown as a horizontal bar. ($P < 10^{-28}$, Wilcoxon rank sum test).
...
In this particular gene expression dataset we did not have the representation of these particular GO categories that were related to hub connectivity in mouse, with only one gene ('unc-32') related to GO:22222 (name) previously found in mouse. 
 
While we were not able to directly test the hypothesis presented by Fulcher et al.\cite{Fulcher2016}, we were interested to see if any connectivity and metabolism related categories will be enriched in the corresponding link types. 
...
As listed in Table~\ref{tab:connected}, the majority of TOP GO categories are related to glutamate signaling and neuronal connectivity ($p < 0.05$).\\
...
On the other hand, both metabolic and glutamate signaling related categories were expected for feed-in links. 
In fact, for both feed-in and feed-out links chemical transmission, metabolism and cholinergic synaptic transmission GO categories were represented among the top 15 categories ($p < 0.05$) while no glutamate signaling GO categories were found.
This could be explained by the fact that only glutamate receptor signaling , but not direct glutamate release GO categories were represented in this particular dataset (no glutamate GO category with at least 5 genes). 

 l
[[TODO-AA: Still kind of surprising we don't get cholinergic, since ACh is still the dominant neurotransmitter for connections involving hubs. Can you look at cholinergic categories -- do we even have gene coverage to get power to find them??AA: on our gene list (from 900) genes related to Ach are either Ach-gated channels or Ach receptors, so it's not about hubs, it's more about neurons that receive inputs from hubs. For reduced lists of genes there is even less Ach-related genes left. ]]
...
\begin{tabular}{lll}
\hline
Neuron & Degree, k & Function                                                     \\ \hline
