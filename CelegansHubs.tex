...
Despite using a directed synaptic connectome (rather than a symmetrized version including gap junctions in Towlson et al.~\cite{Towlson:2013gf}), we see a positively-skewed degree distribution containing an extended tail of high-degree interneurons that overlap almost completely with the list of Towlson et al.~\cite{Towlson:2013gf} (the list of hub neurons for the connectome used here is in Table~\ref{tab:HubList}).
...
We also repeated the analysis with two different definitions of weighted rich-clubs [[CITE]], which also yielded similar results (shown in Fig.~\ref{fig:S_weightedRC}).
...
We use a hub definition in this work of $k > 41$, which corresponds to the lowest degree threshold at which the network displays significant rich-club organization (permutation test, $P < 0.05$).
