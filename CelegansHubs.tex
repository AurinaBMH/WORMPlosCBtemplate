...
Two dimensional spatial coordinates for neurons were obtained as \texttt{celegans277.mat} from the Dynamic Connectome Lab \url{www.biological-networks.org/?page_id=25}.
...
These annotations were assigned to individual neurons using the anatomical hierarchy defined in WormBase, which we retrieved using the WormBase API (wormmine) \cite{\cite{Harris:2009kd}}, propagating each term down the hierarchy to individual neurons.
...
To identify the optimal modular assignment, the algorithm was repeated 1\,000 times, with the final assignments of nodes to modules given as either (i) the solution with maximal modularity, $Q$, or (ii) using a consensus clustering approach \cite{Lancichinetti2012a}, where each partition was weighted by its modularity, $Q$ (using the \texttt{agreement\_weighted} and \texttt{consensus\_und} functions of the Brain Connectivity Toolbox \cite{Rubinov2010}).
