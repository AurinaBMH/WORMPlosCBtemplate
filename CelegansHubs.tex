...
Using open data on the chemical and electrical connectivity of 279 \emph{C. elegans} neurons, and binary gene expression data for each neuron across 948 genes, we computed a correlated gene expression score for each pair of neurons, providing a measure of their gene expression similarity.
...
Some elements of neural systems possess many more connections than others, marking them network hubs. 
...
Full distributions of $r_\phi$ for each edge type at a hub threshold of $k > 44$ are in Fig.~\ref{fig:coExp}A (right).
...
\begin{figure}[h]
  ...
  \textbf{A} CGE values, $r_\phi$, plotted as a function of Euclidean separation distance for all pairs of neurons within the head (gray dots), with a fitted exponential trend shown in black, $f(x) = A\exp(-\lambda x) + B$.
    ...
    This indicates that the trend is not a bulk, isotropic effect, but may instead be driven primarily by a small number of neuron pairs with high $r_\phi$ at short distances ($\lessapprox 25\mu$m), indicated with a circle in \textbf{A}.
    }
