...
Two dimensional spatial coordinates for neurons were obtained as \textt{celegans277.mat} from the Dynamic Connectome Lab \url{www.biological-networks.org/?page_id=25}.
...
Coordinates for three neurons (AIBL, AIYL, SMDVL) were missing in this dataset, and were reconstructed based on the positions of the contralateral neurons (AIBR, AIYR, SMDVR) by assigning identical coordinates for the contralateral neurons in these three pairs. [[according to X? Not in Varier and Kaiser. Should explain the precedent for doing this -- can we somehow check whether these contralateral neurons close?]].
...
These annotations were assigned to individual neurons using the anatomical hierarchy defined in WormBase, which we retrieved using the WormBase API (wormmine), propagating each term down the hierarchy to individual neurons.
...
In this section, we describe the methods used to characterize the \emph{C. elegans} synaptic connectome, represented as a graph with neurons as nodes and synaptic connections as edges.
