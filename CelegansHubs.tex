...
and polymodal neurons (performing more than one type of circuit function) \cite{White:1986tx}.
...
Positions for 3 neurons not included in this dataset (AIBL, AIYL, SMDVL) were reconstructed based on the positions of the contralateral neurons (AIBR, AIYR, SMDVR) by assigning identical coordinates for the contralateral neurons in these three pairs.
...
\textbf{Modularity}
Modular structure of the connectome was determined applying Louvain community detection algorithm using a standard practice, implemented in Brain Connectivity Toolbox. 
...
[[TODO: we need to describe what test we used to determine, and perhaps show the results in supplement. I'm happy to prepare this if you remind me the code that generates. NOTE: I can also do that. The code is compareProportions(binaryCorrelation)]]
In contrast, other measures such as Jaccard index, Yule's Q are highly dependent on this ratio.
[[TODO: Aurina, if we distinguish Yules Q/Y, we need to clearly define, otherwise should we just use the Yule's coefficient defined in Kaufman? LETS use Kaufman's (it's Yule's Q, Yule's Y is the same, but with square roots on each term]]
...
[[TODO: Aurina, adjust record for our PNAS paper, listed as \textbf{Fulcher2016}, but is a 2016 article...CHANGED in my Mendeley citation key]]
Given that connection probability is also dependent on spatial separation in mammalian brains \cite{Henderson:2014fg, Horvat:2016ia}, [[TODO: has this analysis been done in worm? If not we need to do it...SEE evernote: https://www.evernote.com/shard/s94/nl/2147483647/61e44674-7716-4153-a462-28993706d345/]], it is important to correct for spatial effects to ensure that relationships between gene expression and connectivity are not simply the result of both properties showing spatial autocorrelation.
...
[[TODO: add supplementary figure on distance dependence; perhaps coloring by head/tail, SEE: https://www.evernote.com/shard/s94/nl/2147483647/4deb4e43-d986-4bea-b99f-f84f5494d17b/]]
...
[[TODO: Aurina, isn't it more relevant to describe the neurons that we include in the present analysis? 118 neuron classes gives 302 neurons, but we analyze fewer, right? RIGHT: couldn't find the number of classes in somatic NS, so maybe just avoid saying 118]]
...
[[TODO: Aurina, can you comment in the code through the text that you used to produce results like this? I think it would be good to include a standard deviation here. CODE used - testLeftRightCoexpression.m. Distribution for these links is not normal. Median is 1;]]
...
[[AURINA: did we only look for biological processes, or also cellular components, or molecular pathways? ONLY BIOLOGICAL PROCESSES]]
...
Median coexpression also increases slightly for feeder links from around the same threshold $k=38$, while the curve for peripheral links remain flat through the range of degrees.[[NOTE: coexpression for feedin and feedout links differ - should we talk about both or focus on a combined (feed-both) case?]]

 \begin{figure}[!h]
 \centering
    \includegraphics[width=0.7\textwidth]{coexpressionANDdistance}
...
The main goal of this analysis was to compare coexpression within and between modules in order to examine if neurons in the same module tend to be more genetically similar to themselves then to neurons in other modules. 
Despite the fact that M3 showed higher coexpression than both M1 and M2, overall coexpression within modules did not exceed coexpression between modules, demonstrating that neurons belonging to the same module do not share any particular genetic similarity. 
Therefore, higher coexpression in rich and feeder links can not be attributed to the modular organisation of the connectome. 
...
\textbf{Lineage} information presents another source of information about the genetic makeup of the \textit{C. elegans} nervous system. 
...
Top: Degree distribution. Bottom: Median lineage distance for rich, feeder, and peripheral connections as a function of \textit{k}, with the median across all network links shown as a dashed black line and the topological rich club regime shaded grey.
...
\label{S3_Fig}
\centering
    \includegraphics[width=1\textwidth]{CoexpDist}
\end{figure}


\paragraph*{S4 Fig.}
{\bf Connection probability as a function of distance}
\begin{figure}[!h]
\label{S4_Fig}
\centering
    \includegraphics[width=0.7\textwidth]{ProbDist}
\end{figure}
...
\label{S4_Fig}
\centering
    \includegraphics[width=1\textwidth]{RCcurveFINAL2}
\end{figure}
