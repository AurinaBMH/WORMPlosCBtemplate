...
First, we investigated how gene coexpression relates to connectivity by examining it between neuron pairs with different types of connectivity: (i) reciprocally connected, where both neurons send and receive a connection (ii) unidirectionally connected, where only one neuron sends or receives a connection and (iii) unconnected. \\
As it is shown in Fig \ref{RecUnidirUncon}, there is a gradual increase in gene coexpression with increasing level of connectivity.
Reciprocally connected pairs of neurons have more similar expression profiles than both unidirectionally connected (Welch’s t test, $P < 0.05$) and unconnected pairs (Welch’s t test, $P < 10^{-5}$).
Consequently, unidirectionally connected pairs show much higher gene coexpression than unconnected pairs (Welch’s t test, $P < 10^{-18}$).\\
\begin{figure}[!h]
   \centering
    \includegraphics[width=0.9\textwidth]{RecUnidirUnconFINAL}
 \caption{{\bf Coexpression between connected and unconnected neurons.} 
Distribution of transcriptional similarity for all pairs of neurons, grouped by reciprocally connected, unidirectionally connected and unconnected pairs. P-values are from pairwise comparisons using Welch’s t test.}
 \label{RecUnidirUncon}
 \end{figure}
To investigate which functional groups of genes contributed to the difference in transcriptional coupling between connected and unconnected neurons, we used a method of assigning each gene a GCC measure that quantifies the contribution of each gene (see Methods).  \\ 

%% comment on enrichment analysis. No write up can be done before deciding on options. 
\textbf{NOTE: should decide on enrichment analysis as results change when different options are used}. 
\textbf{Scores in case connected and rich+feeder change in magnitude. 
There are 4 options:
\begin{itemize}
    \item{\textbf{run ORA using the full set of genes in both cases:}}
    \begin{itemize}
    \item{with the same threshold (0.0001)? (both connected and RF results give similar enrichment, RF gives more items, but no metabolism). }
    \item{with different thresholds (0.0001 for connected and 0.01 for RF), some metabolism enrichment for RF emerges. lower threshold for RF as score range is different.}
    \end{itemize}
    \item{\textbf{run GSR on}}
    \begin{itemize}
    \item{full gene sample. connected and RF give same categories. weird scores assigned to half of RF genes.}
    \item{If genes with weird scores excluded, connected gives some glutamate categories (619 genes in the sample), RF (425 genes in the sample) don't give any significant categories.}
    \end{itemize} 
\end{itemize} }
...
In this work we use a directed binary connectivity matrix of chemical synapses provided by Varshney et al. \cite{Varshney2011} (see \textit{Materials and Methods}) for 279 non-pharingeal neurons (nodes) and 1961 connections between them (edges). \\
Here we confirm a rich-club organisation of the chemical synapse network, where rich-club organisation manifests across a continuous range of degrees $41<k<58$ ($P<0.05$, shaded grey area in Fig.\ref{RCcoef}) as normalised rich-club coefficient $\Phi_{norm}>1$. 
...
For the second part of our analysis, we focused on the features of rich-club organization of \textit{C. elegans} connectome by investigating the relationship between gene coexpression and neuronal connectivity by comparing three different classes of neuron pairs.
...
Then at each degree, we calculated mean coexpression values for each link type (Fig. \ref{RCdegree}). 
Coexpression for rich links consistently increases for a range of degrees and reaches plateau at around $k=40$ where topological rich club regime begins. 
Mean coexpression also increases slightly for feeder links from $k=52$, while peripheral links steady show decrease before reaching the minimal value.
...
Top: degree distribution. Mean gene coexpression for rich, feeder, and peripheral connections as a function of \textit{k}, with the mean across all network links shown as a dashed black line and the topological rich club regime shaded grey. 
...
Coexpression ($\phi$) for different types of network connections, where hubs are neurons with degree $k \geq 42$. P-values are from pairwise comparisons using Welch’s t test. MAYBE SHOW THIS IN SUPP}
 \label{ViolinPlots}
 \end{figure}
...
glutamate receptor & 0.003 & synaptic transmission & 0.041\\ \hline
cell1 row3 & cell2 row 3 & cell3 row 2 & cell4 row 2\\ \hline
cell1 row4 & cell2 row 4 & cell3 row 2 & cell4 row 2\\ \hline
cell1 row5 & cell2 row 5 & cell3 row 2 & cell4 row 2\\ \hline
cell1 row6 & cell2 row 6 & cell3 row 2 & cell4 row 2\\ \hline
cell1 row7 & cell2 row 7 & cell3 row 2 & cell4 row 2\\ \hline
\end{tabular}
...
\textbf{Lineage and hub connectivity.}\\

\subsection*{EXTRAS: neuron type}
\begin{itemize}
