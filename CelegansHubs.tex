...
While overall there is no clear relationship between lineage distance and gene coexpression in \textit{C. elegans} nervous system (r = -0.017
p = 0.355), all hub neurons are formed early in development, hence it is possible that they originate from common ancestry cells and therefore might share some intrinsic similarities in gene expression that influence the observed increase in coexpression between hub neurons.
...
However, as shown in Fig.~\ref{fig:OtherInfluences}E, there is no significant difference in lineage distance between hub (rich connections) and non-hub neurons (non-rich connections) ($P = 0.0786$, Wilcoxon rank sum test).
...
Nervous system of the \textit{C. elegans} is subdivided into distinct functional groups of neurons performing specialized roles in its behaviour \cite{Hobert2003}.  
Interestingly, ten out of 16 hub neurons with the highest degree correspond to the functional circuit that governs forward (AVBL, AVBR, PVCL, PVCR) and backward (AVAL, AVAR, AVDL, AVDR, AVEL, AVER) locomotion and together are labelled as command interneurons. 
Given their similarity in function we ask if these neurons as a group are more similar in their gene expression than other hub interneurons.
To test that we compare gene coexpression between all pairs of hubs that are command interneurons (n=10) and all pairs of hubs that do not belong to this functional group (n=6).
Indeed, coexpression between command interneurons is significantly higher than between other hub neurons ($p<10^{-7}$, Wilcoxon rank sum test) Fig.~\ref{fig:OtherInfluences}E.
