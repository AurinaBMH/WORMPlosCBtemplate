...
For example, we observe similar results when analysing just the directed chemical connectivity network.  
...
(iv) demonstrate that highly correlated expression between connected hub neurons is not driven by factors like stereotypical interneuron expression, birth time, lineage similarity, neuromodulator types or expression similarity within modules, but may be driven by the high CGE of command interneurons;
...
Thus, we found no evidence for bulk spatial relationships of $r_\phi$ in the neuronal connectome of \emph{C. elegans} and accordingly analyze $r_\phi$ without any bulk spatial correction throughout this work.
...
Our list of hubs overlaps completely with the list of 11 hub neurons in Towlson et al.~\cite{Towlson2013} at $3 \sigma$ (see Table~\ref{tab:HubList}) with a few additional hubs identified in our analysis due to a differences in the version of the connectome that has been analysed.
...
Specifically, we selected a subset of nonhub interneurons that most closely resemble the anatomical properties of hub interneurons, in terms of their position and projection pattern (AVFL, AVFR, AVHL, AVHR, AVKR, AVJL, AVJR).
As shown in Fig.~\ref{fig:interneuron_dep}C, pairs of hub interneurons exhibit a tendency towards slightly higher CGE than pairs of anatomically-matched nonhub interneurons (Wilcoxon rank sum test, $p = 0.051$), suggesting that the increase in CGE amongst hub interneurons probably cannot be explained only by their anatomical similarity among interneurons.
...
As shown in Fig.~\ref{fig:OtherInfluences}C, we also do not find a significant difference in lineage distance between hub (rich connections) and nonhub neurons (non-rich connections) (Wilcoxon rank sum test, $p = 0.079$).
...
One of the best-characterised functional classes that is particularly relevant for our analysis is the set of `command interneurons' which are a functional group of ten neurons that govern forward (AVBL, AVBR, PVCL, PVCR) and backward (AVAL, AVAR, AVDL, AVDR, AVEL, AVER) locomotion \cite{Rakowski2013}; all of these neurons are hubs.
...
as well as genes that are involved in determining cell polarity (\emph{vang-1}, \emph{prkl-1}) \cite{Wu2006, Hoffmann2010} were also amongst the top scoring genes for connectivity.
...
In order to investigate which specific genes contribute most to the increase in CGE for connections involving hubs, we again manually investigated the list of around 20\% highest scoring genes ($p_\mathrm{corr} < 10^{-4}$).
...
While current data are limited, more comprehensive expression annotations in the future would allow more systematic and statistically powered inferences across biological process GO categories.
...
The methods we develop here for quantifying CGE, and scoring genes for their contribution to it, yield biologically interpretable results on incomplete, binary gene expression data.
...
Some of the spatial trends in correlated gene expression found in the 948 genes analyzed here mirror these trends of bulk regions of macroscopic mammalian brains.
