...
Secondly, we compare connections between neurons connected via chemical synapses \textit{vs} gap junctions. 
These two categories are not mutually exclusive, one pair of neurons can share both chemical synapses as well as gap junctions between them, therefore neuron pairs that are connected with both types of synapses (n=188) are accounted for twice. 
Here, $p$ is the proportion of synaptic connections between neurons (0.8575), the maximum number of matches, $n$, is the number of neuron pairs, $(i,j)$, for which $g^{(a)}_i = 1$ and $g^{(a)}_j = 1$ and are also connected.
...
[[TODO: Add labels about head/tail because the data don't actually follow the spatial scale shown on the left]]\\
[[TODO: Check gene coexpression plot is correct ordering]IT IS]\\}
