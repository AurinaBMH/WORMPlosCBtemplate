...
We developed a method to score genes according to their contribution to a given coexpression pattern (see \emph{Methods}). These scores were used to perform an overrepresentation enrichment analysis using gene ontology (GO) categories for biological processes \cite{Ashburner2000, Gillis2010}.
...
\begin{itemize}
     \item Given no GO categories are significantly overrepresented in the enrichment analysis, we investigated the individual genes that contributed towards the increased coexpression.
     \item Several biologically relevant gene groups including glutamate receptor genes (glr-1, glr-2, glr-4, glr-5, nmr-1, nmr-2), innexins (unc-9, unc-7, inx-7, inx-19, inx-13), cell adhesion molecules and transcription factors were found among the top 20$\%$ of the most influential genes. 
Here we show that adhesion molecules related to locomotion (cam-1, unc-5 \cite{Harris:2009kd}) cell migration (cam-1 \cite{Garriga1999}, unc-6, rig-1, unc-5 \cite{Leung-Hagesteijn1992}), axon outgrowth and guidance (sax-3 \cite{Zallen1999}, cam-1 \cite{Garriga1999}) as well as cell polarity (vang-1 \cite{Hoffmann2010}, prkl-1 \cite{Wu2006}) and learning (casy-1 \cite{Ikeda2008}) are implicated.
In addition, transcription factors regulating neuronal development, fate specification (lin-11 \cite{Sarafi-Reinach2001}, unc-3 \cite{Prasad2008}, unc-42 \cite{Baran1999}, ceh-14 \cite{Cassata2000}, ast-1 \cite{Schmid2006}, cfi-1 \cite{Shaham2002a}) and locomotion (unc-3 \cite{Prasad2008}) also play an important role in the increased coexpression between connected neurons. \\
     ...
     \item glutamate ???.
     \item innexins are responsible for the formation of electrical synapses. Here we use the connectome with both chemical and electrical synapses. 
     \item cell adhesion is regulating cell interaction with the surface or other cells, therefore potentially facilitate signal transduction and communication. 
     \item transcription factors found in this analysis mainly regulate neuronal development and fate specification which is important in establishing connections between neurons. 

 \end{itemize}

 INTERPRETATION OF SINGLE GENE RESULTS
...
\begin{itemize}
     \item Here we again look at the top 20$\%$ of the individual genes implicated in the increased coexpression for connections involving hubs. 
     \item In addition to glutamate receptor (glr-5, nmr-1, nmr-2, glr-1, glr-2, grld-1) and acetylcholine related genes (ace-2, cho-1, unc-17, deg-3), we again find a high number of  adhesion molecules (cam-1, rig-1, rig-6, unc-6, grld-1, dbl-1, ncam-1) and transcription factors (unc-3, unc-42, ast-1, pdfr-1) that at least partially overlap with genes found comparing connected and unconnected pairs of neurons.  \\
     \item rig-1, ncam-1 and grld-1 are known to be human orthologs \cite{Harris:2009kd} for genes regulating cell migration, differentiation and neuron cell adhesion. \\
     INTERPRETATION
     
     \item glutamate is known to be important in the regulation of locomotion in command interneurons \cite{Brockie2006}.
     \item majority of hubs are cholinergic (our findings)
     \item high overlap between adhesion molecule and transcription factor genes shows the importance of establishing connections.
 \end{itemize}
 
REMOVE interpretation of enrichment. 
