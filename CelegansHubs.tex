...
These brain network hubs are densely interconnected in what is known as rich-club organization.
Recent work in mouse has identified a distinctive transcriptional signature of neural hubs, characterized by tightly coupled expression of genes regulating oxidative metabolism, with similar genes characterizing macroscale inter-modular hub regions of the human cortex.
Here, we sought to determine whether hubs of the \emph{C. elegans} connectome, which has been mapped at the microscale of individual neurons and synapses, also show tightly coupled gene expression.
Using open data on the chemical and electrical connectivity of 279 \emph{C. elegans} neurons, and binary gene expression expression data for each neuron across 948 genes, we computed a correlated gene expression score for each pair of neurons, providing a measure of their gene expression similarity.
...
Genes with the greatest contribution to this effect are involved in glutamate receptor, acetylcholine signaling as well as neuronal communication related processes.
...
Rich-club organization, whereby a small number of highly central neural elements (hubs) are densely interconnected between themselves, is thought to play an important role of integration in brain networks.
The fact that rich-club organisation manifests in different species and across multiple spatial scales implies its genetic origin. 
Mapping of the \textit{C. elegans} nervous system together with publicly available gene expression datasets enable us to tackle the genetic basis for hub connectivity in more detail.
Here we find that, amongst connected pairs of neurons, pairs of connected hubs are the most similar in their gene expression patterns.
This effect cannot be explained by a range of neuron properties such as neuron type, position, birth time, lineage or neurotransmitter system, but may be linked to the functional class of command interneurons and is driven by coupled expression of genes regulating glutamate, acetylcholine and neuronal communication related genes.
This relationship between hub connectivity and correlated gene expression mirrors results observed in the mesoscale mouse connectome (with hubs also showing distinctive transcriptional patterns in the macroscale human cortex), suggesting that it may be a highly conserved relationship that transcends species and resolution scales.
...
In contrast to this general trend, connected pairs of hubs (i.e., the ``rich club'' of the brain) show the highest levels of transcriptional coupling (compared to hub-nonhub and nonhub pairs), despite being separated by larger distances, on average, and being distributed across broad anatomical brain divisions.
...
We also took an advantage of detailed information on neuron spatial positions, birth times, neuronal lineage as well as the functional and chemical composition of each neuron to understand the mechanisms through which gene expression might influence network topology.
Paralleling findings in humans and mouse, we find that hub neurons of \emph{C. elegans} are transcriptionally distinct.
In particular, as in the mouse brain, connections between hub neurons are most similar in their gene expression.
...
We demonstrate that this effect is not explained by factors such as neuronal birth time, lineage, neurotransmitter system or spatial position but is rather related to the functional specialization of hub neurons, and may be related to their role in mediating higher order behaviours of the organism.
...
Note that all data used for analysis in this work were obtained from publicly available sources, and can be downloaded from an accompanying figshare repository (\url{https://figshare.com/s/797199619fbabdab8c86}).
Code to process this data and reproduce all figures and analyses presented here is on github [and will be made public on acceptance of this paper].
...
Our primary aim in this work is to understand how pairwise patterns of neuronal connectivity (shown in Fig.~\ref{fig:SchematicRepresentation}A) relate to coupled expression across 948 genes between pairs of neurons (i.e., pairs of rows of Fig.~\ref{fig:SchematicRepresentation}C).
...
While these other binary correlation metrics exhibited strong bias with the proportion of gene expression annotations made to a given neuron, $r_\phi$ was not biased (see Fig.~\ref{fig:S_propOnes}). 
...
This measure was not biased by differences in the relative number of expressed genes (Fig.~\ref{fig:S_propOnes}D) and yields qualitatively similar outputs to $r_\phi$ on our data.
...
Note that our basic findings replicated regardless of whether we use $r_\phi$ or this new measure to quantify CGE (e.g., cf. Fig.~\ref{fig:RFPposmatches}).
...
Connections are colored according to how they connect hubs ($k > 44$) and nonhubs ($k \leq 44$), as `rich' (hub $\rightarrow$ hub), `feed-in' (nonhub $\rightarrow$ hub), `feed-out' (hub $\rightarrow$ nonhub), and `peripheral' (nonhub $\rightarrow$ nonhub).
  \textbf{B} Neurochemistry (types as labeled), anatomical location (as labeled), birth time (from early born neurons, black, to late-born neurons, white), hub assignment (hubs labeled red), and functional type (as labeled).
...
Connection probability is plotted as a function of Euclidean separation distance in Fig.~\ref{fig:connectionProb} for each combination of source and target neuron labels, across 10 equiprobable distance bins (with exponential fits added for visualization).
...
CGE decreases slightly with separation distance for the spatially close neurons within the head (Fig.~\ref{fig:spatialCoexpressionMain}A) and within the tail (Fig.~\ref{fig:spatialCoexpressionMain}B), but not for pairs of neurons involving the body (Fig.~\ref{fig:spatialCoexpressionMain}C).
...
Accordingly, attempting to correct for a bulk, non-specific trend by taking residuals from an exponential fitted to the relationship produced artifactual reductions in the CGE of many neuron pairs (shown in Fig.~\ref{fig:S_head_rphi_correct}).
...
% <<hubs>>
Next we analyze the topological properties of the \emph{C. elegans} connectome, represented as a directed, binary connectivity matrix between 279 neurons, combining directed chemical synapses and undirected electrical gap junctions (Fig.~\ref{fig:SchematicRepresentation}).
% between 279 neurons.
...
Our list of hubs includes all of the 11 hub neurons of Towlson et al.~\cite{Towlson2013} at $3 \sigma$ (see Table~\ref{tab:HubList}) with a few additional hubs identified in our analysis of the directed connectome.
...
This can be attributed to a relative increase in long-distance hub-hub connections between the head and tail, shown in Fig.~\ref{fig:neuronsSpace} (cf. Fig.~\ref{fig:S_connectionProportion}).
% , we find an increase in mean hub-hub connection distance with $k$, through to the extent of the topological rich-club regime.
The high connection density and long mean anatomical distance between pairs of hub neurons counters the general trend in the \emph{C. elegans} connectome, where the probability of connectivity between two neurons decays with their separation distance (Fig.~\ref{fig:connectionProb\texttt{}).}
...
We next investigate how the network connectivity properties of \emph{C. elegans} relate to patterns of correlated gene expression, using the mean square contingency coefficient, $r_\phi$.
...
(ii) all pairs of neurons that are connected via a chemical synapse (1940 pairs), and
...
Specifically, we selected a subset of nonhub interneurons that most closely resemble the anatomical properties of hub interneurons, in terms of their position and projection pattern: AVFL, AVFR, AVHL, AVHR, AVKR, AVJL, and AVJR.
As shown in Fig.~\ref{fig:interneuron_dep}C, pairs of hub interneurons show higher median CGE than pairs of anatomically-matched nonhub interneurons, with the difference being at the threshold of statistical significance (Wilcoxon rank sum test, $p = 0.051$), suggesting that the increase in CGE amongst hub interneurons probably cannot be explained by their anatomical similarity among interneurons.
...
Circles indicate a statistically significant increase in CGE in a given link type relative to the rest of the network (one-sided Wilcoxon rank-sum test, $p < 0.05$).
\textbf{C} CGE distributions for all pairs of hub interneurons (red) and all pairs of anatomically matched nonhub interneurons (dark yellow) (Wilcoxon rank sum test, $p = 0.051$).
...
%As shown in Fig.~\ref{fig:interneuron_dep}B, pairs of hub interneurons exhibit significantly higher CGE than pairs of anatomically-matched nonhub interneurons (Wilcoxon rank sum test, $p = 0.047$).
...
% There was no significant difference in CGE for connections within modules compared to those between modules ($p = 0.69$, Wilcoxon rank sum test), as shown in Fig.~\ref{fig:OtherInfluences}B.
...
\textbf{D} Distributions of CGE, $r_\phi$, between early born hubs (rich links, red) and nonhubs (non-rich links, blue) shown as violin plots with the median shown as a horizontal bar (Wilcoxon rank sum test, $p = 3.9 \times 10^{-22}$).
\textbf{E} Distributions of CGE between hub command interneurons (red) and hub non-command interneurons (blue) shown as violin plots with the median shown as a horizontal bar (Wilcoxon rank sum test, $p = 3.3 \times 10^{-8}$).
...
As shown in Fig.~\ref{fig:OtherInfluences}C, we also do not find a significant difference in lineage distance between pairs of connected hubs (rich connections) and other types of connections (non-rich) (Wilcoxon rank sum test, $p = 0.079$).
...
% TypeConfoundHubs.m
To determine whether the similarity in CGE between pairs of hub neurons could be explained by their neurotransmitter types, we constructed $10^8$ random sets of sixteen neurons of the same neurotransmitter types as hubs (e.g., thirteen random cholinergic neurons, two random glutamatergic neurons, and one random neuron of unknown neurotransmitter type) and compared the distribution of median CGE of each group to the median CGE of hub neurons as a permutation test.
...
The methods we develop here for quantifying CGE, and scoring genes for their contribution to it, yield biologically interpretable results from incomplete binary gene expression data.
...
The effect cannot be driven by the modular organization of the network, since even though most hubs resided in the same topological module, we found that CGE between hubs in the same module is higher than between other pairs of neurons in the same topological module, with a similar increase in CGE for pairs of hubs in different modules.
...
Association areas sit at a top of the cortical hierarchy and support complex behaviors by integrating information from diverse neural systems \cite{Mesulam1998}.
...
While we were not able to test GO categories related to neurotransmitter signaling, due to insufficient coverage of gene expression annotations, single gene analysis revealed the importance of acetylcholine genes, which may be related to the fact that acetylcholine is the dominant neurotransmitter in hub neurons.
...
We thank WormBase and especially Daniela Raciti for providing some valuable information regarding gene expression data.
Thanks to Taqi Ali for preliminary data processing.
...
We verified that this index yields qualitatively similar results to the mean square contingency coefficient, $r_\phi$, used throughout this work (Fig.~\ref{fig:RFPposmatches}), validating our related positive match method for scoring individual genes [as an approximately single-gene contribution to the probability score computed in Eq.~\eqref{eqn:pmatch}].
...
% ------------------------------------------------------------------------------
% \paragraph*{S1 Fig.}
% {\bf Comparing different CGE measures.}
\begin{figure}[h]
\centering
...
% RichClub script
% \paragraph*{S10 Fig.}
% {\bf Average CGE for rich, feeder and peripheral links as a function of degree.}
\begin{figure}[!h]
\centering
    \includegraphics[width=1\textwidth]{meanCoexpressionCoXALL.pdf}
\caption{
\textbf{Correlated gene expression measured using the positive matching probability index.}
...
\emph{Bottom}: Mean CGE calculated using similarity index from only positive matches, $p_match$, for each connection type as a function of $k$.
...
\begin{figure}[h]
  ...
  \caption{
  \textbf{Correcting for spatial effects in CGE data using a bulk exponential trend.}
  ...
  \textbf{A} CGE values, $r_\phi$, plotted as a function of Euclidean separation distance for all pairs of neurons within the head (gray dots), with a fitted exponential trend shown in black, $f(x) = A\exp(-\lambda x) + B$.
