...
The plot reveals rich-club organization at the upper tail of the degree distribution, particularly for thresholds $41 < k < 58$, shaded grey in Fig.\ref{fig:topology_rich}B.
...
We define rich-club hubs here as the 13 neurons with $k > 41$.
...
At each node degree, $k$ (x-axis in Fig \ref{RCdegree} B), we defined each neuron as either a hub (nodes with degree $\geq k$) or a nonhub (otherwise), and then labelled each connection as rich (hub $\rightarrow$ hub), feeder (nonhub $\rightarrow$ hub or hub $\rightarrow$ nonhub), or peripheral (nonhub $\rightarrow$ nonhub).
...
% RichClubMedian script
 ...
 \caption{{\bf Coexpression and topology}
\textbf{A: }Distribution of transcriptional similarity between connected and unconnected neurons.
$P<10^{-50}$, Wilcoxon rank sum test. 
\textbf{B: }Top: degree distribution. Middle: proportion of link types at each degree threshold. Bottom: Mean gene coexpression for rich, feeder, and peripheral connections as a function of \textit{k}, with the mean across all network links shown as a dashed black line and the topological rich club regime shaded grey.
